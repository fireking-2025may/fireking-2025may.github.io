...
(function () {
  ...
  function splitTextNode(block, fit) {
    ...
    while (low <= high) {
      ...
      editable.textContent = text.slice(0, middle);
      if (fit()) { best = middle; low = middle + 1; } else high = middle - 1;
    }
    if (!best) { editable.textContent = text; return null; }
    const breakAt = text.lastIndexOf(' ', best);
    best = breakAt > best / 2 ? breakAt + 1 : best;
    editable.textContent = text.slice(0, best);
    const rest = block.cloneNode(true);
    rest.querySelector('[contenteditable=true]').textContent = text.slice(best);
