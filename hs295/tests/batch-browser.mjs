...
await columns.nth(1).locator('[name=transferValueEntry]').fill('');await page.getByRole('button',{name:'Save all transfers',exact:true}).click();if(await page.locator('#transfer-list .card').count()!==0)throw Error('Invalid batch was partially saved');if(await page.locator('.batch-column-error').count()<1)throw Error('Invalid column was not identified');await columns.nth(1).locator('[name=transferValueEntry]').fill('25');
...
page.once('dialog',d=>d.accept());const [session]=await Promise.all([page.waitForEvent('download'),page.locator('.export-session').first().click()]);const sessionPath=path.join('/tmp','hs295-batch-session.json');await session.saveAs(sessionPath);await page.reload();await page.setInputFiles('#import-file',sessionPath);page.once('dialog',d=>d.accept());await page.waitForTimeout(50);if(await page.locator('#transfer-list .card').count()!==2)throw Error('Batch session did not import');
