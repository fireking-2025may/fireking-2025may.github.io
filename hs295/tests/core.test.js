...
test('normalises identifiers and postcode',()=>{assert.equal(C.normalizeUTR('reference text'),'reference text');assert.equal(C.normalizeNINO('ab 12 34 56 c'),'AB123456C');assert.equal(C.normalizePostcode('SW1A 1AA'),'SW1A 1AA')});
test('UTR accepts free text and remains optional',()=>{for(const identifierValue of ['', '   ', '123456789', 'Estate reference A/7'])assert.doesNotMatch(C.personErrors({...people[0],identifierValue}).join(),/UTR/)});
