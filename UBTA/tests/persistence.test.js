...
test('a stale tab cannot overwrite or rotate a newer draft',()=>{const storage=new MemoryStorage(),p=factory(storage),record=p.create(seedDocument),first=p.saveDraft(record),newer=p.saveDraft({...first,document:{...first.document,meta:{...first.document.meta,projectTitle:'Newer tab'}}});const recoveryBefore=storage.getItem(p.keys(record.documentId).recovery);assert.throws(()=>p.saveDraft({...first,document:{...first.document,meta:{...first.document.meta,projectTitle:'Stale tab'}}},'pagehide'),error=>error.code==='REVISION_CONFLICT'&&error.expectedRevision===1&&error.storedRevision===2);assert.equal(p.load(record.documentId).document.meta.projectTitle,'Newer tab');assert.equal(storage.getItem(p.keys(record.documentId).recovery),recoveryBefore);assert.equal(p.load(record.documentId).revision,newer.revision)});
...
test('failed storage writes do not replace a previous valid draft',()=>{const storage=new MemoryStorage(),p=factory(storage),record=p.create(seedDocument),saved=p.saveDraft(record);const before=storage.getItem(p.keys(record.documentId).draft),original=storage.setItem.bind(storage);storage.setItem=(key,value)=>{if(key.endsWith(':temporary:v1'))throw Error('quota');original(key,value)};assert.throws(()=>p.saveDraft({...saved,document:{...saved.document,meta:{...saved.document.meta,projectTitle:'Lost'}}}),/quota/);assert.equal(storage.getItem(p.keys(record.documentId).draft),before)});
